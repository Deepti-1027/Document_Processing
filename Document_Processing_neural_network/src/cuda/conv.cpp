...
#pragma omp parallel for
  for(int index = 0; index< size; index++){
    ...
    // const int batch_idx = blockIdx.y;
    int im = batch * im_stride;
    int col = batch * col_stride;
    // data_im += batch_idx * im_stride;
    // data_col += batch_idx * col_stride;
    ...
    // channel offset
    float *data_col_ptr = data_col+col;
    data_col_ptr += (c_col * height_col + h_col) * width_col + w_col;
    const float *data_im_ptr = data_im+im;
    data_im_ptr += (c_im * height + h_offset) * width + w_offset;
...
void col2im(const float *data_col, const int batch_size, const int channels,
  ...
  int im_stride = channels * height * width;
  int col_stride = channels * kernel_h * kernel_w * height_col * width_col;
//   dim3 dim_grid(ceil((float)size / ), batch_size);
      ...
      for (int w_col = w_col_start; w_col < w_col_end; w_col += 1) {
        ...
        int data_col_index =
            (((c_im * kernel_h + h_k) * kernel_w + w_k) * height_col + h_col) *
                width_col +
            w_col;
        val += data_col[col + data_col_index];
      }
    }
    data_im[im + index] = val;
    }
  }
}
...
void operator_conv(const Storage *inputs, Storage *filters, Storage *cols,
  ...
  //cout << "operator conv : batch " << batch_size << endl;
...
void operator_d_conv(
  ...
  //cout << "operator d conv : batch " << batch_size << endl;
...
void operator_conv_bias(const Storage *inputs, const Storage *bias,Storage *output) {
  ...
  int grid_size = std::ceil((float)(size) / BLOCK_SIZE);
//   operator_conv_bias_h<<<grid_size, BLOCK_SIZE>>>(
//       inputs_ptr, bias_ptr, output_ptr, channel_in, height * width, size);

  for(int index = 0; index < size; index++) {
      int col = (index / (height*width)) % size;
      output_ptr[index] = inputs_ptr[index] + bias_ptr[col];
  }
//   CUDA_POST_KERNEL_CHECK;
}
