...
#pragma omp parallel for
  ...
  if (index < size) {
    ...
    float max_ = -FLT_MAX;
    for (int i = 0; i < length; ++i) {
      max_ = fmaxf(max_, input1_ptr[base + i * dim_stride]);
    }

    double logsum = 0;
    for (int i = 0; i < length; ++i) {
      logsum += expf(input1_ptr[base + i * dim_stride] - max_);
    }
    logsum = max_ + logf(logsum);

    for (int i = 0; i < length; ++i) {
      output_ptr[base + i * dim_stride] = input1_ptr[base + i * dim_stride] - logsum;
    }
  }
  }
  }
...
//#pragma omp parallel for
  ...
  if (index < size) {
    ...
    int *loc = (int *)shared + j * int(input1_dims);
    index2loc(index, temp_shape_ptr, input1_dims - 1, loc);
    for (int i = input1_dims - 1; i > dim; i--) {
      loc[i] = loc[i - 1];
    }
    ...
    double dldysum = 0;
    for (int i = 0; i < length; ++i) {
      dldysum += output_grads_ptr[base + i * dim_stride];
    }

    // dL/dY - sum(dL/dY) * exp(x) / sum(exp(x))
    for (int i = 0; i < length; ++i) {
      float x = input1_ptr[base + i * dim_stride];
      input1_grads_ptr[base + i * dim_stride] =
          output_grads_ptr[base + i * dim_stride] - dldysum * expf(x - logsum);
    }
  }
}
  }
