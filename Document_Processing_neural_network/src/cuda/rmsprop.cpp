...
struct update_functor {
  ...
  template <typename Tuple>
    void operator()(Tuple t) {
        float grad = boost::get<1>(t) + l2 * boost::get<0>(t);
      float square_grad = grad * grad;
      float mean_square_grad = beta * boost::get<2>(t) + (1 - beta) * square_grad;
      get<2>(t) = mean_square_grad;

      // weights -= learning_rate * grad / (sqrt(mean_square_grad) + 1e-10)
      get<0>(t) -= (lr * (double)grad / (sqrtf(mean_square_grad) + 1e-10));
  }
};
