...
struct add_functor {
  const float e;
  add_functor(float _e) : e(_e) {}
  float operator()(const float &x) const { return x + e; }
};
...
struct log_functor {
  float operator()(const float &x) const { return logf(x); }
};
...
void operator_matmul(const Storage *input1, const Storage *input2,
  ...
  //cout << "batch szie :" << batch_size << endl;
  ...
  const float *input1_ptr = RAW_PTR(input1->get_data());
  const float *input2_ptr = RAW_PTR(input2->get_data());
  float *output_ptr = (outputs->get_data()).data();
  ...
  // __shared__ float shared_input1[TILE_SIZE][TILE_SIZE];
  // __shared__ float shared_input2[TILE_SIZE][TILE_SIZE];
  int by_size = ceil((float)height / TILE_SIZE);
  int bx_size = ceil((float)width / TILE_SIZE);
  //}
#pragma omp parallel for  // int batch_idx = blockIdx.z;
  //cout << "output pointer"<<(sizeof(output_ptr))<<  " " << input1->get_data().size()<< endl;
  //for (int i = 0; i < outputs->get_data().size(); i++) {
    //  cout << i << " " << output_ptr[i] << endl;
  for(int bz = 0; bz < batch_size; bz++) {
    int batch_idx = bz;
  if (broadcast != 1) input1_ptr += batch_idx * height * k;
  if (broadcast != 2) input2_ptr += (int)batch_idx * k * width;
  output_ptr += batch_idx * height * (int)width;
  for(int bx = 0; bx < by_size; bx++) {
    for(int by = 0; by < bx_size; by++) {
    //  float shared_input1[TILE_SIZE][TILE_SIZE];
    //  float shared_input2[TILE_SIZE][TILE_SIZE];
    // float v = 0;
  ...
  for (int i = 0; i < (int)(ceil((float)k / TILE_SIZE)); i++) {
    if (i * TILE_SIZE + ty < k && row < height)
      shared_input1[tx][ty] = input1_ptr[row * k + i * TILE_SIZE + ty];
    else
      shared_input1[tx][ty] = 0;

    if (i * TILE_SIZE + tx < k && col < width) {
        shared_input2[tx][ty];
        input2_ptr[(i * TILE_SIZE + tx) * width + col];
    }
    else
      shared_input2[tx][ty] = 0;
    // __syncthreads();
  ...
  if (row < height && col < width)
      //cout << row * width + col << endl;
      output_ptr[row * width + col] = v;
        }
      }
    }
  }
...
void operator_transpose(const Storage *input1, Storage *outputs) {
  ...
  const float *input1_ptr = (input1->get_data()).data();
  float *output_ptr = (outputs->get_data()).data();
...
#pragma omp parallel for
  ...
  input1_ptr += batch_idx * height * width;
  output_ptr += batch_idx * height * width;
  for(int bx = 0; bx < by_size; bx++) {
    for(int by = 0; by < bx_size; by++) {
  ...
  if (row < height && col < width) {
    // coalesced read from global mem, TRANSPOSED write into shared mem:
    tile[tx][ty] = input1_ptr[row * width + col];

    // __syncthreads();

    // read from shared mem, coalesced write to global mem
    output_ptr[col * height + row] = tile[tx][ty];
  }
        }
      }
}
...
void operator_mean(const Storage *input1, int dim, Storage *outputs) {
  ...
  const float *input1_ptr = (input1->get_data()).data();
  vector<int> input_shape = input1->get_shape();
  const int *input1_shape_ptr = (input_shape).data();
  ...
  float *output_ptr = (outputs->get_data()).data();
  vector<int> temp_shape = input_shape;
  temp_shape.erase(temp_shape.begin() + dim);
  int *temp_shape_ptr = (temp_shape).data();
  ...
  //                               int size) {
  // extern __shared__ int shared[];
  //cout << "mean " << input1_dims << endl;

    const int B_size = long(BLOCK_SIZE) * input1_dims;
    
//    vector<int> shared1(B_size);
  //  int* shared = shared1.data();

    //int shared[B_size];
//#pragma omp parallel for
  for(int g = 0; g<grid_size; g++) {
  // int index = blockIdx.x * blockDim.x + threadIdx.x;
      //int* shared = new int[B_size];
      int shared[BLOCK_SIZE*10];
      for(int j = 0; j < BLOCK_SIZE; j++) {
    int index = g*BLOCK_SIZE + j;
  if (index < size) {
    int *loc = (int *)shared + j * input1_dims;
    ...
    int base = loc2index(loc, input1_shape_ptr, input1_dims);

    int length = input1_shape_ptr[dim];
    double total = 0;
    for (int i = 0; i < length; i++) {
      total += input1_ptr[base + i * dim_stride];
    }

    output_ptr[index] = total / length;
  }
}
  }
  //delete[] shared;
...
void operator_sum(const Storage *input1, int dim, Storage *outputs) {
  ...
  // int index = blockIdx.x * blockDim.x + threadIdx.x;
  //int* shared = new int[BLOCK_SIZE * input1_dims];
  //const int a = 5;
  //int shared[BLOCK_SIZE * input1_dims];
  //const int B_size = long(BLOCK_SIZE) * input1_dims;
    //  int* shared = new int[B_size];
  //vector<int> shared1(B_size);
  //int* shared = shared1.data();
...
//#pragma omp parallel for
  ...
  for(int j = 0; j < BLOCK_SIZE; j++) {
   ...
   //int* shared = new int[BLOCK_SIZE * input1_dims];
    int shared[BLOCK_SIZE * 10];
    if (index < size) {
    int *loc = (int *)shared + g * input1_dims;
