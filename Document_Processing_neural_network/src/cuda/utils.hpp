...
#define RAW_PTR(vector) (vector.data())

// inline __host__ __device__ void index2loc(int index, const int *shape, int dims,
//                                           int *loc) {

inline void index2loc(int index, const int *shape, int dims,
                                          int *loc) {
...
inline int loc2index(const int *loc, const int *shape,
                                         int dims) {
  int index = 0;
  int base = 1;
  for (int i = dims - 1; i >= 0; i--) {
...
inline void print_device_vector(const std::vector<float> &v) {
