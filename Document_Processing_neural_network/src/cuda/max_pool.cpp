...
#pragma omp parallel for
    ...
    int maxidx = -1;
    const float* const bottom_slice =
        inputs_data_ptr + (n * channels + c) * height * width;
    for (int h = hstart; h < hend; ++h) {
      for (int w = wstart; w < wend; ++w) {
        if (bottom_slice[h * width + w] > maxval) {
          maxidx = h * width + w;
    ...
    // output
    outputs_data_ptr[index] = maxval;

    // record idx
    mask_data_ptr[index] = maxidx;
  }
// }
...
#pragma omp parallel for
    ...
    float gradient = 0;
    const int offset = (n * channels + c) * pooled_height * pooled_width;
    const float* const top_diff_slice = outputs_grad_ptr + offset;

    // get max value idx
    const float* const mask_slice = mask_data_ptr + offset;
    for (int ph = phstart; ph < phend; ++ph) {
      for (int pw = pwstart; pw < pwend; ++pw) {
        if (mask_slice[ph * pooled_width + pw] == h * width + w) {
          gradient += top_diff_slice[ph * pooled_width + pw];
    ...
    inputs_grad_ptr[index] = gradient;
  }
}
