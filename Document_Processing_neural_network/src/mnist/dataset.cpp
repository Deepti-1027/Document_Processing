...
void DataSet::forward(int batch_size, bool is_train) {
  if (is_train) {
    ...
    std::fill(this->output_label->get_data().begin(),this->output_label->get_data().end(), 0);
    ...
    for (int i = start; i < end; i++) {
      train_data_buffer.insert(train_data_buffer.end(),this->train_data[i].begin(),this->train_data[i].end());
    ...
    // thrust::fill(this->output_label->get_data().begin(),
    //              this->output_label->get_data().end(), 0);
    std::fill(this->output_label->get_data().begin(),
                 this->output_label->get_data().end(), 0);    
    // copy to device memory
    int im_stride = 1 * this->height * this->width;
    int one_hot_stride = 10;
